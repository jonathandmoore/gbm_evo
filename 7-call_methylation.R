...
# 28/10/2019
#
# Version 1 - 1001 methylomes data analysis, adapted from Schmitz/Becker project - call methylation at all sites for a single sample
#
# Change log
#
...
# Jay's Dec 2019 sequences (5 of)
sample_list3 = read.table(paste0("../0-raw_data/","jay_samples.txt"), sep="\t", header=TRUE)   
fastq_list3 = read.table(paste0("../0-raw_data/","jay_sequences.txt"), sep="\t", header=TRUE)  

sample_list = rbind.data.frame(sample_list1, sample_list2, sample_list3)
fastq_list = rbind.data.frame(fastq_list1, fastq_list2, fastq_list3)
...
for (this_run in fastq_list[fastq_list$experiment_accession==sample_id,"run_accession"]) {
	...
	# accumulate C and T values from each sample, taking care of NAs which occur when sample has zero coverage
